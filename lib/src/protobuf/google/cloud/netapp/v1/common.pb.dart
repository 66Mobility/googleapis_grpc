...
class LocationMetadata extends $pb.GeneratedMessage {
  factory LocationMetadata({
    $core.Iterable<ServiceLevel>? supportedServiceLevels,
  }) {
    final $result = create();
    if (supportedServiceLevels != null) {
      $result.supportedServiceLevels.addAll(supportedServiceLevels);
    }
    return $result;
  }
  ...
  static final $pb.BuilderInfo _i = $pb.BuilderInfo(_omitMessageNames ? '' : 'LocationMetadata', package: const $pb.PackageName(_omitMessageNames ? '' : 'google.cloud.netapp.v1'), createEmptyInstance: create)
    ..pc<ServiceLevel>(1, _omitFieldNames ? '' : 'supportedServiceLevels', $pb.PbFieldType.KE, valueOf: ServiceLevel.valueOf, enumValues: ServiceLevel.values, defaultEnumValue: ServiceLevel.SERVICE_LEVEL_UNSPECIFIED)
  ...
  @$pb.TagNumber(1)
  $core.List<ServiceLevel> get supportedServiceLevels => $_getList(0);
}
